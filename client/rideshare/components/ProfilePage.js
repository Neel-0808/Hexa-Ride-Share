  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setGender(gender);
        setProfilePicture(profile_picture ? `http://192.168.215.164:4000/${profile_picture}` : null); // Formatted URI for the profile picture
        ...
        <View style={tw`relative`}>
          <TouchableOpacity onPress={pickImage}>
            <Image
              ...
              style={tw`w-24 h-24 rounded-full border-4 border-black-100`}
            />
          </TouchableOpacity>
        </View>
